\chapter{Cyclostationary signal processing: an approach to categorising signal families}
...
\section{Using WSC signals to create a cache heirarchy}
...
\subsection{LUT generation for arbitrary signals}
...
\[
\arccos(x) = 2 \, \arccos\!\Big(\frac{1}{\sqrt{2}} \, \sqrt{x+1}\Big).
\]
...
T_1 = (x+1)(x^2+x+1)
\]
can be implemented in \textbf{2 multiplications} and \textbf{2 additions} as follows:
...
\[
T_2 = -4 x^2 + 7 x + 15 = -(x^2 << 2) + ((x+1 + 1) << 3) - (x+1)
\]
...
\begin{itemize}
    \item 2 multiplications (for $T_1$)
    \item 4 additions/subtractions (2 for $T_1$, 1 for $T_2$, 1 for the final sum)
    \item 2 bit-shifts and 1 bitwise inversion
\end{itemize}

\paragraph{Advantages:}
\begin{enumerate}
    \item No transcendental operations in the argument apart from the $\frac{1}{\sqrt{2}}$ multiplier, simplifying quantization.
    \item Only a single DSP unit is required if the data width allows single-cycle multiplication.
    \item No additional storage for constants is needed.
    \item The computation is easily pipelined.
    \item Bit-growth is predictable, scaling relative to the highest degree of the polynomial.
\end{enumerate}

\subsubsection*{Insights and Extensions}
