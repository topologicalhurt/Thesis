...
\subsection{Setup: Walsh basis and discrete (dyadic) synthesis}

Let \(\{W_k\}_{k\ge 0}\) denote the Walsh–Paley system on \([0,1)\), an orthonormal basis of \(L^2([0,1))\) \cite{Paley1932,Fine1949,GES1991}.
For any \(f\in L^2([0,1))\), its Walsh expansion is
\[
...
Fix an integer \(m\ge 1\) and set \(N:=2^m\). Let the dyadic grid be
\[
x_j := \frac{j}{N},\quad j=0,1,\dots,N-1.
\]
Define the (vector of) samples \(y_j := f(x_j)\).
The \emph{discrete} Walsh–Hadamard synthesis uses the \(N\times N\) Hadamard matrix \(H_N\) with entries \((H_N)_{k,j} = W_k(x_j)\in\{\pm 1\}\) (Paley ordering).
Then the length-\(N\) coefficient vector \(c = (c_k)_{k=0}^{N-1}\) is
\[
c = \frac{1}{N} H_N\, y,
\qquad \text{i.e. }
c_k = \frac{1}{N}\sum_{j=0}^{N-1} y_j\,W_k(x_j).
\]
The length-\(N\) partial Walsh sum is the step function
\[
S_N(x) := \sum_{k=0}^{N-1} c_k\,W_k(x),
\]
which equals the conditional expectation of \(f\) onto the dyadic \(\sigma\)-algebra at scale \(2^{-m}\) (the average of \(f\) on each dyadic subinterval).

\subsection{Forcing dyadic coefficients via dyadic sampling \& quantization}

\paragraph{Step 1 (Dyadic quantizer).}
Fix \(b\in\mathbb{N}\). Define the dyadic quantizer \(Q_b:\mathbb{R}\to 2^{-b}\mathbb{Z}\) by
\[
Q_b(u) := 2^{-b} \,\mathrm{round}(2^b u).
\]
Apply \(Q_b\) to the samples:
\[
\tilde y_j := Q_b\big(f(x_j)\big) \in 2^{-b}\mathbb{Z}.
\]

\paragraph{Step 2 (FWHT / discrete coefficients).}
Compute
\[
\tilde c_k := \frac{1}{N}\sum_{j=0}^{N-1} \tilde y_j\, W_k(x_j).
\]
Since each \(\tilde y_j\) is a dyadic rational and \(W_k(x_j)\in\{\pm 1\}\), we have
\[
\tilde c_k \in 2^{-(b+m)}\mathbb{Z}
\quad\Rightarrow\quad
\tilde c_k \ \text{is dyadic},\ \ \forall k.
\]
Thus the \emph{quantized discrete Walsh expansion}
\[
\tilde S_N(x) := \sum_{k=0}^{N-1} \tilde c_k\, W_k(x)
\]
has \emph{dyadic} coefficients and is piecewise constant on dyadic intervals.

\subsection{Worked example: $f(x)=\sin(2\pi x)$}

Take \(f(x)=\sin(2\pi x)\), which has the non-dyadic Fourier series
\(f(x)=\frac{1}{2i}\big(e^{2\pi i x}-e^{-2\pi i x}\big)\) (coefficients are not dyadic).
Choose \(m,b\) and define
\[
x_j=\frac{j}{2^m},\qquad
\tilde y_j = Q_b\big(\sin(2\pi x_j)\big).
\]
Compute Walsh coefficients
\[
\tilde c_k = 2^{-m}\sum_{j=0}^{2^m-1} \tilde y_j\, W_k\!\Big(\frac{j}{2^m}\Big)\in 2^{-(b+m)}\mathbb{Z}.
\]
Hence the Walsh approximation \(\tilde S_{2^m}(x)=\sum_{k=0}^{2^m-1}\tilde c_k W_k(x)\) is a \emph{dyadic series} that replaces the non-dyadic trigonometric series.

\subsection*{Error decomposition and guarantees}

Write three errors:
\[
\|f-\tilde S_N\|_{L^2}
\le
\underbrace{\|f - \mathbb{E}[f\,|\,\mathcal{D}_m]\|_{L^2}}_{\text{dyadic averaging error}}
\ +\
\underbrace{\|\mathbb{E}[f\,|\,\mathcal{D}_m]-\mathbb{E}[\tilde f\,|\,\mathcal{D}_m]\|_{L^2}}_{\text{sample quantization}}
\ +\
\underbrace{\|\mathbb{E}[\tilde f\,|\,\mathcal{D}_m]-\tilde S_N\|_{L^2}}_{\text{discrete vs. continuous Walsh}},
\]
where \(\mathcal{D}_m\) is the dyadic \(\sigma\)-algebra at scale \(2^{-m}\) and \(\tilde f\) is the function matching the quantized samples on grid points and extended piecewise constantly on each dyadic interval.

\begin{itemize}
\item (\emph{Averaging error})
\(\mathbb{E}[f\,|\,\mathcal{D}_m]\) is the best \(L^2\)-approximation of \(f\) by functions constant on dyadic intervals.
As \(m\to\infty\), \(\|f - \mathbb{E}[f\,|\,\mathcal{D}_m]\|_{L^2}\to 0\) (martingale convergence; see \cite[Chs.~1--2]{GES1991}).
If \(f\) is Lipschitz with constant \(L\), then
\[
\|f - \mathbb{E}[f\,|\,\mathcal{D}_m]\|_{L^\infty}\le L\,2^{-m}.
\]

\item (\emph{Quantization error})
Since \(|\tilde y_j - y_j|\le 2^{-b-1}\),
\[
\|\mathbb{E}[f\,|\,\mathcal{D}_m]-\mathbb{E}[\tilde f\,|\,\mathcal{D}_m]\|_{L^2}
\ \le\ 2^{-b-1}.
\]

\item (\emph{Discrete/continuous Walsh match})
For \(N=2^m\), the rectangle rule over dyadic intervals is \emph{exact} for Walsh atoms \(W_k\) (they are constant on each interval), hence
\[
\mathbb{E}[\tilde f\,|\,\mathcal{D}_m](x)
= \sum_{k=0}^{N-1} \tilde c_k\, W_k(x)
= \tilde S_N(x),
\]
...
\ \ \|f-\tilde S_N\|_{L^2}
\ \le\
\|f - \mathbb{E}[f\,|\,\mathcal{D}_m]\|_{L^2}
\ +\ 2^{-b-1},
\ }
\]
and every coefficient \(\tilde c_k\) is dyadic: \(\tilde c_k\in 2^{-(b+m)}\mathbb{Z}\).
